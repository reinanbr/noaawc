...
def _add_features(ax: plt.Axes, lw: float = 0.4, show_states: bool = False) -> None:
...
def _draw_info_box(
    ...
    Two modes, selected by the ``ax`` parameter:

    ``ax=None``  (default — OrthoAnimator / globe)
        ``fig.text()`` in figure coordinates.
        The orthographic globe fills the figure squarely, so figure-coord
        placement is reliable and consistent.

    ``ax=<Axes>``  (PlateCarreeAnimator / any flat projection)
        ``ax.text()`` with ``ax.transAxes``.
        Anchors the box to the TOP-RIGHT corner of the map Axes, keeping it
        inside the map frame regardless of aspect ratio, colorbar padding, or
        figure margins.

    Parameters
    ----------
    fig      : active Figure
    var_key  : variable key shown in the box  (e.g. "t2m")
    cycle    : GFS cycle string               (e.g. "00z")
    date_str : pre-formatted frame date       (e.g. "2026-04-17")
    scale    : DPI-proportional font scale factor
    ax       : supply for flat projections (PlateCarree, Mercator, …);
               omit for OrthoAnimator
    x, y     : figure-coord position — only used when ``ax=None``
    """
    lines = [
...
def _draw_data_credit(
    ...
    ``ax=None``   → figure coordinates (OrthoAnimator)
    ``ax=<Axes>`` → axes coordinates, bottom-right of the map frame
                    (PlateCarreeAnimator / flat projections)
    """
    props = dict(
        ha="right",
        va="bottom",
...
# Latitudes with cartographic meaning
_REF_LATITUDES = {
...
# Longitudes: Prime Meridian + International Date Line + main continental anchors
_REF_LONGITUDES = {
...
def _add_reference_lines(ax: plt.Axes, lw: float = 0.4) -> None:
    """
    Draw key geographic reference lines on a cartopy axes:

    Parallels
    ---------
    - Equator              (solid blue,   slightly thicker)
    - Tropics of Cancer / Capricorn  (dashed amber)
    - Arctic / Antarctic Circles     (dotted ice-blue)

    Meridians
    ---------
    - Prime Meridian  0°   (solid grey)
    - Date Line     180°   (dashed grey)
    - 90°W / 90°E          (faint dashed grey)

    All lines are drawn in PlateCarree coordinates so they project correctly
    onto any Cartopy projection (Orthographic, Mercator, etc.).
    Lines are intentionally subtle — they add geographic context without
    competing with the data field.

    Parameters
    ----------
    ax : plt.Axes with a Cartopy projection
    lw : base linewidth (scales with the ``scale`` factor passed in from
         _add_features).  Default 0.4.
    """
    ...
    # ── parallels ─────────────────────────────────────────────────────────────
    for key, cfg in _REF_LATITUDES.items():
        lat_val = cfg["lat"]
        ax.plot(
            lon_range,
            np.full_like(lon_range, lat_val),
            transform=transform,
            color=cfg["color"],
            linewidth=lw * cfg["lw_factor"],
            linestyle=cfg["ls"],
            alpha=cfg["alpha"],
            zorder=2,
        )
    ...
    # ── meridians ─────────────────────────────────────────────────────────────
    for key, cfg in _REF_LONGITUDES.items():
        lon_val = cfg["lon"]
        ax.plot(
            np.full_like(lat_range, lon_val),
            lat_range,
            transform=transform,
            color=cfg["color"],
            linewidth=lw * cfg["lw_factor"],
...
class OrthoAnimator:
    ...
    View shortcut (lon / lat / zoom in one call)
    --------------------------------------------
    anim.set_view(lon=-50.0, lat=-15.0)           # recentre, keep zoom
    anim.set_view(lon=-50.0, lat=-15.0, zoom=2)   # recentre + zoom
    ...
    def set_states(self, visible: bool = True) -> "OrthoAnimator":
        """
        Enable or disable state/province boundary lines on the map.

        Uses the Natural Earth admin_1_states_provinces shapefile (10m scale),
        with global coverage. Cartopy downloads it automatically on first use
        and caches it locally.

        Disabled by default — enable for regional views (zoom >= 2) where
        state boundaries help with geographic orientation.

        Parameters
        ----------
        visible : bool
            True  → render state/province boundaries  (default when calling the method)
            False → hide state/province boundaries
        ...
        Examples
        --------
        anim.set_states()           # enable
        anim.set_states(True)       # same
        anim.set_states(False)      # disable

        # method chaining
        (OrthoAnimator(ds, "t2m", central_point=(-40.5, -9.4))
            .set_zoom(4)
            .set_states()
            .set_quality("hd")
            .animate())
    ...
    # ── zoom option ───────────────────────────────────────────────────────────────
    def set_zoom(self, zoom: float) -> "OrthoAnimator":
        """
        Zoom into the orthographic globe around the current central point.

        zoom=1 (default) shows the full visible hemisphere.
        Higher values crop progressively tighter around the central point
        by narrowing the angular radius of the extent.

        The angular radius is computed as:
            radius_deg = 90.0 / zoom

        So zoom=2 → 45° radius, zoom=4 → 22.5° radius, zoom=8 → ~11° radius.

        The extent is applied in PlateCarree coordinates, clipped to the
        orthographic projection, which means you always see a globe-shaped
        view — not a flat rectangular crop.

        Parameters
        ----------
        zoom : float
            Zoom multiplier.  Must be ≥ 1.

            ========  =============  ====================================
            Value     Radius         Coverage (approx.)
            ========  =============  ====================================
            1         90°            Full hemisphere (default behaviour)
            2         45°            Continental scale  (e.g. South America)
            4         22.5°          Sub-continental    (e.g. Northeast Brazil)
            8         11.25°         Regional           (e.g. Ceará + neighbours)
            16         5.6°          State-level
            ========  =============  ====================================
        ...
        Examples
        --------
        # Continental zoom on South America, centred on Juazeiro-BA
        anim = OrthoAnimator(ds, "t2m", central_point=(-40.5, -9.4))
        anim.set_zoom(3)
        anim.animate()
        ...
        # Method chaining
        (OrthoAnimator(ds, "prmsl", central_point=(-50.0, -15.0))
            .set_zoom(2)
            .set_quality("hd")
            .set_author("Maria Silva")
            .animate())
        ...
        """
        if zoom < 1:
            raise ValueError("zoom must be >= 1 (1 = full hemisphere, higher = tighter).")
    ...
    def set_view(
        ...
        """
        Set the central point and optional zoom level in a single call.

        This is the OrthoAnimator equivalent of
        ``NearsidePerspectiveAnimator.set_view()``, providing a unified API
        across both animators.  Instead of a ``satellite_height`` parameter
        (which is specific to the Nearside Perspective projection), Ortho
        exposes ``zoom`` — a dimensionless multiplier that crops the visible
        hemisphere around the central point.
        ...
        zoom : float or None
            Zoom multiplier ≥ 1.  ``None`` (default) leaves the current zoom
            unchanged.

            The angular radius of the visible area is computed as::
        ...
        Examples
        --------
        Recentre only, keep current zoom::

            anim.set_view(lon=-40.5, lat=-9.4)

        Recentre and apply continental zoom::

            anim.set_view(lon=-50.0, lat=-15.0, zoom=2)

        Tight regional view over Northeast Brazil::

            anim.set_view(lon=-40.5, lat=-9.4, zoom=6)

        Method chaining::

            (OrthoAnimator(ds, "t2m")
                .set_view(lon=-50.0, lat=-15.0, zoom=2)
                .set_quality("hd")
                .set_author("Maria Silva")
                .animate())
        ...
        Notes
        -----
        ``set_view()`` updates ``_central_point``, which is used both as the
        projection centre and as the camera rest position when no rotation arc
        is defined via ``set_rotation()``.  If a rotation arc was previously
        configured, the new central point does not affect the arc endpoints —
        call ``set_rotation()`` again to update those explicitly.
        """
        self._central_point = (float(lon), float(lat))
        if zoom is not None:
            if zoom < 1:
                raise ValueError(
                    "zoom must be >= 1 (1 = full hemisphere, higher = tighter)."
                )
            self._zoom = float(zoom)
        return self
    ...
    def _build_axes(self, central: tuple) -> tuple[plt.Figure, plt.Axes]:
        ...
        if self._zoom > 1.0:
            lon_c, lat_c = central
            r = 90.0 / self._zoom          # angular radius in degrees
            ax.set_extent(
                [
                    lon_c - r,             # west
                    lon_c + r,             # east
