...
class NearsidePerspectiveAnimator:
    ...
    def _get(self, time_idx: int):
        ...
        # NOTE: VARIABLE_PRESETS["convert"] runs here at render time.
        # VARIABLES_INFO["converter"] runs once at data-load time in the
        # pipeline — both should NOT convert the same quantity (e.g. K→°C).
        # If VARIABLES_INFO already applied the conversion, omit "convert"
        # from the preset for that variable to avoid double-application.
        convert = VARIABLE_PRESETS.get(var, {}).get("convert", None)
        if convert is not None:
            data = convert(data)
        mask_below = VARIABLE_PRESETS.get(var, {}).get("mask_below", None)
    ...
    def _draw_annotations(self, ax, lat, lon, field) -> None:
        if not self._annotations:
            return
        scale = _font_scale(self._dpi)
        # Matches real printf-style format specs (%d, %.1f, %e, …) only.
        # A bare "%" used as a unit symbol (e.g. "80%" or "(%)") does NOT
        # match, so variables like r2 whose label contains "%" don't trigger
        # a spurious "incomplete format" ValueError.
        # Edge case: "RH: %d%%" — the escaped "%%" is handled correctly by
        # Python's % operator before this regex ever sees the result string,
        # so double-percent escapes in text_base work as expected.
        ...
        for ann in self._annotations:
            ...
            if ann["interpolate"] and _FMT_RE.search(ann["text_base"]):
                ...
                if np.isnan(val):
                    val = 0.0  # fallback for points outside the convex hull of the data (e.g. near the limb)
    ...
    def _draw_field(
        ...
        cmap.set_under(alpha=0)  # values below first level → transparent
        ...
        # FIX: Skip contour lines for patchy / categorical / binary variables.
        # These fields (precipitation, snow, categorical precip types, radar
        # reflectivity, cloud cover, sea ice, visibility) produce cluttered or
        # misleading contours because the data is spatially discontinuous or
        # has only a few discrete values.  For all other variables the thin
        # white iso-lines add useful meteorological structure.
