...
def _remove_contours(ax: plt.Axes) -> None:
    ...
    Note: ``LineCollection`` lives in ``matplotlib.collections``, not
    ``matplotlib.contour`` — the latter no longer re-exports it in modern
    matplotlib versions.
    """
...
# Simpler accessor (no transforms) — kept for backward compatibility.
...
class _AnimatorBase:
    """
    Internal base class that implements:
      - all configuration setters shared across the four animators
      - the shared field-rendering pipeline (_draw_field_common, _render_frame, animate, plot)

    Subclasses must define class-level defaults:
        _QUALITY_PRESETS, _OUTPUT_DEFAULT, _FPS_DEFAULT, _STEP_DEFAULT,
        _DPI_DEFAULT, _FIGSIZE_DEFAULT, _CODEC_DEFAULT, _VIDEO_QUALITY_DEFAULT

    And must implement:
        _build_axes(*args, **kwargs) -> (fig, ax)
        _get(time_idx) -> (lats, lons, data, time_val)
        _frames_prefix() -> str          — prefix for the frames directory
        _draw_field_extra(fig, ax, lat, lon, field, time_val, **kw)
                                          — projection-specific pcolormesh + contour
        plot(time_idx, save, show)        — may be overridden if signature differs
    """
    ...
    def _draw_overlays(
        ...
        *,
        ...
        """Draw all shared overlays: annotations, colorbar, title, info box, credit, author.

        Parameters
        ----------
        ax_anchored : bool
            ``True``  → info box and credit are anchored to *ax* (flat projections).
    ...
    def _get(self, time_idx: int):
        """Return (lats, lons, data, time_val) for the given time index."""
        raise NotImplementedError

    def _build_axes(self, *args, **kwargs) -> tuple[plt.Figure, plt.Axes]:
        """Create and return (fig, ax) for one frame."""
        raise NotImplementedError

    def _draw_field_extra(self, fig, ax, lat, lon, field, time_val, cf) -> None:
        """Hook for projection-specific post-pcolormesh drawing (e.g. contours)."""

    def _frames_prefix(self) -> str:
        """Prefix used when naming the per-variable frames directory."""
        raise NotImplementedError
    ...
    def _draw_field(self, fig, ax, lat, lon, field, time_val,
        ...
        Subclasses customise behaviour via:
          - ``_make_norm_and_cmap()`` — returns (norm, cmap) for pcolormesh
          - ``_draw_contour()``       — draws contour lines (or skips them)
          - ``_draw_overlays()``      — shared overlays; pass ax_anchored as needed

        Parameters
        ----------
        _suppress_contours : bool
            Internal flag — when ``True`` the contour step is skipped entirely.
            Used by ``_render_frame`` when retrying after a cartopy
    ...
    def _render_frame(self, tidx: int, fpath: str, **build_kw) -> None:
        """Render one frame to *fpath*.  Extra kwargs are forwarded to ``_build_axes``.

        Cartopy defers contour path re-projection to draw time, so
        ``TypeError: 'GeometryCollection' object is not subscriptable`` can
        surface inside ``fig.savefig`` even when ``ax.contour`` succeeded.
        When that happens the figure's internal render state is corrupted, so
        the only reliable fix is to close it and re-render from scratch with
        contours suppressed via the ``_suppress_contours`` flag on
        ``_draw_field``.
        """
        ...
        try:
            ...
            # Cartopy GeometryCollection bug during deferred path projection —
            # re-render from scratch without contours.
            _do_render(suppress_contours=True)

    def _animate_loop(self, fdir: str, n_frames: int, build_kw_for: Any = None) -> None:
        """Core animation loop — renders missing frames then encodes the video.

        Parameters
        ----------
        build_kw_for : callable(tidx) -> dict, optional
            When provided, called per frame to obtain extra kwargs for ``_build_axes``
            (used by the rotating-globe animators to pass the current camera position).
        """
        for tidx in range(n_frames):
...
class _RotatingAnimatorMixin:
    """
    Mixin that adds camera-rotation support to globe-type animators
    (OrthoAnimator and NearsidePerspectiveAnimator).

    Requires the host class to define ``_lon`` / ``_lat`` (or ``_central_point``
    via a shim) as the default camera position.
    """
    ...
    def _default_camera(self) -> tuple[float, float]:
        """Return the static camera position when no rotation is set."""
        raise NotImplementedError  # implemented differently in each subclass
    ...
    def _camera_at(self, tidx: int, stop: int) -> tuple[float, float]:
        """Interpolate camera position for frame *tidx*."""
...
class _FlatAnimatorMixin:
    """
    Mixin that adds named-region / zoom / ocean / grid setters to the flat
    projection animators (PlateCarreeAnimator and RobinsonAnimator).
    """
    ...
    # ── FIX: use _get_field_full so convert + mask_below are applied ──────────
    def _get(self, time_idx: int):
...
class OrthoAnimator(_RotatingAnimatorMixin, _AnimatorBase):
    ...
    # ── _RotatingAnimatorMixin interface ──────────────────────────────────────
    ...
    # ── _AnimatorBase interface ───────────────────────────────────────────────

    def _frames_prefix(self) -> str:
        return self._var  # ortho uses plain var name (historic convention)
...
class NearsidePerspectiveAnimator(_RotatingAnimatorMixin, _AnimatorBase):
    ...
    def _make_norm_and_cmap(self):
        """Nearside uses transparent under/bad colours for the ocean boundary."""
    ...
    def _draw_contour(self, ax, lat, lon, field) -> None:
        """Skip contours for known no-contour variables; silently drop on TypeError."""
