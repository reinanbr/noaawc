...
def static_nearside_prmsl():
    ...
    anim = WeatherAnimator(
        ds, "prmsl", mode="nearside", lon=-50.0, lat=-15.0, satellite_height=35_786_000
    )
    anim.set_quality("hd")
...
def static_plate_r2_ne_brazil():
    ...
    anim.set_author("@reinanbr_")
    anim.set_annotate(
        "Juazeiro %.0f%%", pos=(-9.4, -40.5), marker="*", marker_color="#f7c948"
    )
...
def anim_cape_satellite():
    ...
    anim = WeatherAnimator(
        ds, "cape", mode="nearside", lon=-50.0, lat=-15.0, satellite_height=10_000_000
    )
    anim.set_output(str(out))
    anim.set_quality("hd")
    anim.set_fps(10)
...
def parse_args():
    p = argparse.ArgumentParser(description="Generate noaawc README example plots.")
    p.add_argument(
        "--only-static", action="store_true", help="Only render static snapshots"
    )
    p.add_argument(
        "--only-animated", action="store_true", help="Only render animated GIFs"
    )
    p.add_argument("--var", default=None, help="Filter by variable key (e.g. t2m)")
    p.add_argument(
        "--mode",
        default=None,
...
def run_jobs(jobs: list, label: str, var_filter: str | None, mode_filter: str | None):
    ...
    for tag, fn in jobs:
        ...
        if var_filter and var_filter not in var_tag:
            continue
        if mode_filter and mode_filter not in mode_tag:
            continue
